...
comic_id_re = compile(r'comic2-(\d+)\.png$')


def get_max_comic_id():
    return comic_id_re.search(BS(get('http://www.qwantz.com/index.php', headers=hdrs).text).select('.comic')[0]['src']).group(1)


def save_panel2(comic_id):
    r = get('http://www.qwantz.com/comics/comic2-{0}.png'.format(comic_id), stream=True)
